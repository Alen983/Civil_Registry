...
con=pymysql.connect("localhost","root","","civil")
c=con.cursor()
...
def cusreg(request):
    ...
    if 'submit' in request.POST:
        ...
        cusid=data12[0]
        con.commit()
    return render(request,'CustomerRegistration.html',{'d':d,'cusid':cusid})
def home(request):
...
def BirthCertificateregistration(request):
    ...
    if 'birthsubmit' in request.POST:
        ...
        if yr>dyr:
            ...
            c.execute(me)
            con.commit()
            return HttpResponseRedirect("/BirthCertificateregistration/")   
